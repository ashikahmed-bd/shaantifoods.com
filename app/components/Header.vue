  ...
  <header class="border-b border-border bg-white">
    <nav class="mx-auto max-w-7xl px-4 py-2">
      <div class="flex items-center justify-between gap-4">
        <button class="flex size-10 items-center justify-center rounded-full border border-border lg:hidden"
          @click="isOpen = !isOpen">
        ...
        <ul class="hidden items-center gap-8 text-sm font-medium uppercase lg:flex">
          <li v-for="item in navigation" :key="item.to">
            <NuxtLink :to="item.to" class="font-semibold transition hover:text-primary" active-class="text-primary">
              {{ item.label }}
            </NuxtLink>
          </li>
        </ul>
      ...
      <Transition enter-active-class="transition-all duration-200" enter-from-class="opacity-0 -translate-y-2"
        ...
        <div v-if="isOpen" class="mt-4 rounded-2xl border border-border bg-white p-4 shadow-sm lg:hidden">
          <ul class="flex flex-col gap-4 uppercase text-sm font-medium">
            <li v-for="item in navigation" :key="item.to">
              <NuxtLink :to="item.to" class="block transition hover:text-primary" active-class="text-primary">
                {{ item.label }}
              </NuxtLink>
            </li>
          </ul>
